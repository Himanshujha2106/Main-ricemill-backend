...
async def do_panding(dopanding: DoPendingBase, db: Session = Depends(get_db)):
...
async def lot_number_master(
    lotnumbermaster: LotNumberMasterBase, db: Session = Depends(get_db)
):
...
async def mohan_food_paddy(
    mohanfoodpaddy: MohanFoodPaddyBase, db: Session = Depends(get_db)
):
